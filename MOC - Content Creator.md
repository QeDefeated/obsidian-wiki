---
title: "MOC - Контент Креатор"
tags:
  - MOC
  - index
  - контент-креатор
---

# MOC - База знаний контент креатора

> Собрано из 10 видео Katie Steckly + ASMR анализ и сценарии — полный гайд по созданию контента

---

## YouTube

### Запуск и стратегия
- [[Start YouTube Channel 2026]] - план запуска, ниша, оборудование, монетизация
- [[YouTube Niche Finance AI Hybrid]] - гибридная ниша Finance × AI для EN-рынка
- [[YouTube AI Tools ES Channel]] - контент-план AI Tools канала (ES, 40 тем, партнёрки)
- [[YouTube Historia Geopolitica ES Channel]] - контент-план Historia/Geopolítica (ES, 40 тем, evergreen)
- [[YouTube Thumbnails]] - дизайн превью для алгоритма 2026

### Съемка
- [[How to Film Yourself]] - оборудование, свет, кадр, звук
- [[Sony Camera Settings]] - 7 хаков для ZV-E1 / ZV-E10
- [[Vlogging Guide]] - сторителлинг из повседневности

### Монтаж
- [[Video Editing Tutorial]] - 10 хаков для DaVinci Resolve, B-Roll, звук

---

## Instagram

### Планирование
- [[Instagram Content Planning]] - контент-столпы, воронка, форматы
- [[Instagram Batch Creating]] - создание недели контента за 3-4 часа

---

## Бренд и Монетизация

- [[Rebrand OnlyFans Guide]] - ребрендинг, видение бренда, 8 элементов, пошаговый план
- [[Brand Model and Funnel]] - воронка YouTube → Patreon → OF → Fansly, тиры, доход
- [[Content Perception Guide]] - улучшение восприятия контента на YouTube и OF

---

## Психология и методология

- [[Kurpatov OF Fact Map]] - факт-карта Курпатова (24 факта: доминанта, дофамин, парасоциальность, Троица, ДСМ)

---

## ASMR Girlfriend / Companion

- [[ASMR Girlfriend Patterns]] - паттерны и выводы из анализа 12 видео
- [[ASMR Genre Analysis]] - структурный анализ жанра, формула, психология
- [[ASMR Video Scripts]] - 5 оригинальных сценариев на русском

---

## Продуктивность

- [[Productivity System]] - 5 правил non-hustle продуктивности
- [[Weekly Productivity Routine]] - недельное расписание Katie

---

## Релокация и иммиграция

- [[Argentina VNZh Guide 2026]] - все виды ВНЖ в Аргентине (транзитная, временная, постоянная, прекариа, Decreto 366/2025)

---

## Тематические связи

```
┌─────────────────────────────────────────────────────────┐
│                                                         │
│  YouTube                    Instagram                   │
│  ┌──────────┐              ┌──────────┐                │
│  │ Стратегия│──────────────│Планирован│                │
│  │ Запуск   │              │ие контент│                │
│  └────┬─────┘              └────┬─────┘                │
│       │                         │                       │
│  ┌────┴─────┐              ┌────┴─────┐                │
│  │ Съемка   │──────────────│  Батч    │                │
│  │ Камера   │              │ Креатинг │                │
│  └────┬─────┘              └──────────┘                │
│       │                                                 │
│  ┌────┴─────┐     ┌──────────────────┐                 │
│  │ Монтаж   │     │  БРЕНД/МОНЕТИЗ.  │                 │
│  │ Превью   │     │  Ребренд+Воронка │                 │
│  └──────────┘     │  OF/Fansly/Patr. │                 │
│                    └──────────────────┘                 │
│            ┌──────────────────┐                         │
│            │  ПРОДУКТИВНОСТЬ  │                         │
│            │  Система + Рутина│                         │
│            └──────────────────┘                         │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

---

## Теги

| Тег | Описание |
|-----|----------|
| `#youtube` | Все о YouTube |
| `#instagram` | Все об Instagram |
| `#продуктивность` | Системы и рутины |
| `#монтаж` | Видеомонтаж |
| `#камера` | Оборудование для съемки |
| `#thumbnails` | Превью |
| `#влог` | Влоги |
| `#стратегия` | Стратегия роста |
| `#batch-creating` | Батч-создание контента |
| `#контент-план` | Планирование контента |
| `#davinci-resolve` | DaVinci Resolve |
| `#sony` | Камеры Sony |
| `#notion` | Notion для креаторов |
| `#workflow` | Рабочие процессы |
| `#work-life-balance` | Баланс работа/жизнь |
| `#asmr` | ASMR контент и анализ |
| `#парасоциальность` | Парасоциальные механизмы |
| `#girlfriend-experience` | ASMR Girlfriend жанр |
| `#сценарии` | Сценарии видео |
| `#бренд` | Брендинг и ребрендинг |
| `#ребренд` | Ребрендинг контента |
| `#onlyfans` | OnlyFans стратегии |
| `#fansly` | Fansly стратегии |
| `#patreon` | Patreon монетизация |
| `#воронка` | Воронка монетизации |
| `#монетизация` | Монетизация контента |
| `#курпатов` | Методология Курпатова |
| `#троица` | Типология «Троицы» |
| `#дсм` | Дефолт-система мозга |
| `#восприятие` | Восприятие контента |
| `#ai-tools` | AI-инструменты и обзоры |
| `#personal-finance` | Личные финансы |
| `#en-market` | Контент для EN-рынка |
| `#es-market` | Контент для ES-рынка |
| `#история` | Исторический контент |
| `#геополитика` | Геополитика и анализ |
| `#ниша` | Выбор и анализ ниши |
| `#аргентина` | Аргентина |
| `#внж` | Виды на жительство |
| `#иммиграция` | Иммиграция и релокация |
| `#резиденция` | Резиденция и визы |
| `#номад` | Цифровые кочевники |

---

## Быстрые действия

### Я хочу начать YouTube канал
1. [[Start YouTube Channel 2026]]
2. [[YouTube Niche Finance AI Hybrid]]
3. [[YouTube AI Tools ES Channel]]
4. [[YouTube Historia Geopolitica ES Channel]]
5. [[How to Film Yourself]]
6. [[Sony Camera Settings]]
7. [[Video Editing Tutorial]]
8. [[YouTube Thumbnails]]

### Я хочу расти в Instagram
1. [[Instagram Content Planning]]
2. [[Instagram Batch Creating]]

### Я хочу построить/обновить бренд
1. [[Rebrand OnlyFans Guide]]
2. [[Brand Model and Funnel]]
3. [[Content Perception Guide]]
4. [[ASMR Girlfriend Patterns]]
5. [[Kurpatov OF Fact Map]]

### Я хочу быть продуктивнее
1. [[Productivity System]]
2. [[Weekly Productivity Routine]]

### Я хочу разобраться в ASMR Girlfriend жанре
1. [[ASMR Girlfriend Patterns]]
2. [[ASMR Genre Analysis]]
3. [[ASMR Video Scripts]]

### Я хочу переехать в Аргентину
1. [[Argentina VNZh Guide 2026]]

### Я хочу улучшить качество видео
1. [[Content Perception Guide]]
2. [[Sony Camera Settings]]
3. [[Video Editing Tutorial]]
4. [[Vlogging Guide]]
5. [[YouTube Thumbnails]]

---

## Источник

Все видео: **Katie Steckly** ([YouTube](https://www.youtube.com/@KatieSteckly)) | [Instagram](https://www.instagram.com/katiesteckly) | [Creatorly Media](https://www.creatorlymedia.com)
